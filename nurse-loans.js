(function () {
  ...
  function recordConsent() {
    consentRecorded.value = "Nurse-loan enquiry processing requested at " + new Date().toISOString() + "; privacy notice version 2026-08-19";
  }
  ...
  function whatsappUrl(payload) {
    const message = [
      "Hello Kredibility Finance, I would like to enquire about a nurse loan.",
      "",
      "Name: " + payload.name,
      "Phone: " + payload.phone,
      "Email: " + payload.email,
      "Employer / facility: " + payload.company,
  ...
  form.addEventListener("submit", async function (event) {
    ...
    try {
      ...
      if (choices && choices.advertising && typeof window.fbq === "function") window.fbq("track", "Lead", { content_name: "Nurse Loans" });
      if (choices && choices.analytics && typeof window.gtag === "function") window.gtag("event", "generate_lead", { method: "nurse_whatsapp_enquiry" });

      status.textContent = "Details saved. Opening WhatsApp…";
      window.location.assign(whatsappUrl(payload));
    } catch (error) {
      console.error("Nurse lead submission error:", error);
