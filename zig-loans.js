(function () {
  ...
  function recordConsent() {
    consentRecorded.value = "ZiG-loan enquiry processing requested at " + new Date().toISOString() + "; privacy notice version 2026-08-19";
  }
  ...
  function whatsappUrl(payload) {
    const message = [
      "Hello Kredibility Finance, I would like to enquire about a ZiG loan.",
      "",
      "Name: " + payload.name,
      "Phone: " + payload.phone,
      "Email: " + payload.email,
  ...
  form.addEventListener("submit", async function (event) {
    ...
    try {
      ...
      if (choices && choices.advertising && typeof window.fbq === "function") window.fbq("track", "Lead", { content_name: "ZiG Loans" });
      if (choices && choices.analytics && typeof window.gtag === "function") window.gtag("event", "generate_lead", { method: "zig_whatsapp_enquiry" });

      status.textContent = "Details saved. Opening WhatsApp…";
      window.location.assign(whatsappUrl(payload));
    } catch (error) {
      console.error("ZiG lead submission error:", error);
