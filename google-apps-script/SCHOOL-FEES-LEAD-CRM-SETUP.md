# School fees lead Google Sheets CRM setup

The school fees loans landing page already delivers each completed lead through Web3Forms before opening WhatsApp. Complete these steps to also copy each lead into a Google Sheet CRM.

1. Create or open the Google Sheet that should hold campaign leads.
2. Select `Extensions` > `Apps Script`.
3. Paste the contents of `school-fees-lead-crm.gs` into the editor and save.
4. Select `Deploy` > `New deployment` > `Web app`.
5. Set `Execute as` to `Me` and `Who has access` to `Anyone`.
6. Deploy, approve the permissions and copy the `/exec` Web app URL.
7. Open `school-fees-loans.html` and paste that URL into the form attribute:
...
9. Confirm that the `School Fees Loan Leads` tab contains the test row and that WhatsApp opens.
